...
/**
 * GET /api/storefront/filters?collection=<handle>
 * Returns filter options + product→filter mappings from DB metafields.
 * Used by FilterBar to populate dropdowns and filter client-side.
 */
export async function GET(request: NextRequest) {
  ...
  for (const row of rows.rows as any[]) {
    ...
    if (row.colours_raw) {
      try { for (const c of JSON.parse(row.colours_raw)) { colours.push(c.handle); allColours.add(c.handle); } } catch {}
    }
    if (row.shapes_raw) {
    ...
    // Parse size from sizing_dimensions (Frame width → S/M/L)
