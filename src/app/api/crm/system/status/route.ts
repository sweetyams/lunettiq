...
async function check(id: string, name: string, fn: () => Promise<string>): Promise<Status> {
  try { return { id, name, status: 'ok', detail: await fn() }; }
  catch (e: any) { return { id, name, status: 'error', detail: e.message?.slice(0, 80) }; }
}
...
async function resolveKey(keyName: string, dbConfigs: Map<string, Record<string, string>>): Promise<string | null> {
  ...
  const id = KEY_MAP[keyName];
  if (id) { const keys = dbConfigs.get(id); if (keys?.[keyName]) return keys[keyName]; }
...
export const GET = handler(async () => {
  ...
  statuses.push(await check('neon', 'Neon Postgres', async () => {
    ...
    return `Connected · ${(r.rows[0] as any)?.c ?? 0} integrations`;
  }));
  ...
  } else statuses.push({ id: 'shopify_storefront', name: 'Shopify Storefront API', status: 'off', detail: 'No token' });

  if (adminToken && shopDomain) {
    statuses.push(await check('shopify_admin', 'Shopify Admin API', async () => {
      const res = await fetch(`https://${shopDomain}/admin/api/2024-04/shop.json`, { headers: { 'X-Shopify-Access-Token': adminToken } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return 'Connected';
    }));
    statuses.push(await check('shopify_webhooks', 'Shopify Webhooks', async () => {
      const res = await fetch(`https://${shopDomain}/admin/api/2024-04/webhooks.json`, { headers: { 'X-Shopify-Access-Token': adminToken } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    statuses.push({ id: 'shopify_admin', name: 'Shopify Admin API', status: 'off', detail: 'No token' });
  }
  ...
  } else statuses.push({ id: 'clerk', name: 'Clerk Auth', status: 'off', detail: 'No key' });
  ...
  if (inngestSigning) {
    ...
    try {
      ...
      statuses.push({ id: 'inngest', name: 'Inngest', status: r.ok ? 'ok' : 'error', detail: r.ok ? 'Dev server' : 'Error' });
    } catch {
  ...
  if (squareToken) {
    ...
    statuses.push(await check('square_webhooks', 'Square Webhooks', async () => {
      const res = await fetch(`${sqBase}/webhooks/subscriptions`, { headers: { Authorization: `Bearer ${squareToken}`, 'Content-Type': 'application/json' } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const d = await res.json();
      const subs = d.subscriptions ?? [];
      if (!subs.length) throw new Error('No subscriptions');
      return `${subs.length} sub · ${subs.flatMap((s: any) => s.event_types ?? []).length} events`;
    }));
  } else {
    statuses.push({ id: 'square', name: 'Square POS', status: 'off', detail: 'No token' });
    statuses.push({ id: 'square_webhooks', name: 'Square Webhooks', status: 'off', detail: 'No token' });
  }
  ...
  } else statuses.push({ id: 'upstash', name: 'Upstash Redis', status: 'off', detail: 'Optional' });

  // Klaviyo
  const klaviyoKey = await key('KLAVIYO_PRIVATE_KEY');
  if (klaviyoKey) {
    statuses.push(await check('klaviyo', 'Klaviyo', async () => {
      const res = await fetch('https://a.klaviyo.com/api/accounts/', { headers: { Authorization: `Klaviyo-API-Key ${klaviyoKey}`, revision: '2024-02-15' } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return 'Connected';
    }));
  } else statuses.push({ id: 'klaviyo', name: 'Klaviyo', status: 'off', detail: 'No key' });

  // Pixels
  for (const p of [
    { id: 'posthog', name: 'PostHog', k: 'NEXT_PUBLIC_POSTHOG_KEY' },
    { id: 'polar', name: 'Polar Analytics', k: 'NEXT_PUBLIC_POLAR_SITE_ID' },
    { id: 'ga4', name: 'Google Analytics 4', k: 'NEXT_PUBLIC_GA4_ID' },
    { id: 'meta_pixel', name: 'Meta Pixel', k: 'NEXT_PUBLIC_META_PIXEL_ID' },
    { id: 'tiktok', name: 'TikTok Pixel', k: 'NEXT_PUBLIC_TIKTOK_PIXEL_ID' },
    { id: 'pinterest', name: 'Pinterest Tag', k: 'NEXT_PUBLIC_PINTEREST_TAG_ID' },
    { id: 'snapchat', name: 'Snapchat Pixel', k: 'NEXT_PUBLIC_SNAPCHAT_PIXEL_ID' },
  ]) {
    const v = await key(p.k);
