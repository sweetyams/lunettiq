...
export const GET = handler(async (request) => {
  ...
  const locationFilter = params.get('location'); // location_id or 'online'
  ...
  const locationClause = locationFilter === 'online'
    ? sql`AND source = 'shopify'`
    : locationFilter
  ...
  const [
    ...
    db.execute(sql`
      ...
      FROM orders_projection WHERE created_at >= ${since} AND created_at <= ${until} ${locationClause}
      GROUP BY source`),
    ...
    db.execute(sql`
      SELECT location_id as location, count(*) as orders,
        coalesce(sum(total_price::numeric), 0) as revenue,
        round(avg(total_price::numeric), 2) as aov
      FROM orders_projection WHERE created_at >= ${since} AND created_at <= ${until} AND location_id IS NOT NULL ${locationClause}
    ...
    db.execute(sql`
      ...
      WHERE created_at >= ${since} AND created_at <= ${until} AND item->>'name' IS NOT NULL ${locationClause}
    ...
    db.execute(sql`
      ...
      FROM (
        ...
        FROM orders_projection WHERE shopify_customer_id IS NOT NULL AND created_at >= ${since} AND created_at <= ${until} ${locationClause}
        GROUP BY 1
      ) sub`),
