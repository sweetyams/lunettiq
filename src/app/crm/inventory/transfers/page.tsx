...
export default function TransfersPage() {
  ...
  const [newLines, setNewLines] = useState<Array<{ familyId: string; colour: string; familyName: string; quantity: number }>>([]);
  ...
  const filtered = search.length >= 1 ? colourOptions.filter(o => `${o.familyName} ${o.colour}`.toLowerCase().includes(search.toLowerCase())) : [];
          ...
          {filtered.length > 0 && (
            <div style={{ border: '1px solid #e5e7eb', borderRadius: 6, maxHeight: 150, overflowY: 'auto', marginBottom: 8, background: '#fff' }}>
              {filtered.slice(0, 15).map(o => (
                <button key={`${o.familyId}-${o.colour}`} onClick={() => { if (!newLines.some(l => l.familyId === o.familyId && l.colour === o.colour)) setNewLines([...newLines, { ...o, quantity: 1 }]); setSearch(''); }} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '6px 10px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 12, borderBottom: '1px solid #f3f4f6' }}>
                  <span style={{ fontWeight: 500 }}>{o.familyName}</span> — {o.colour.replace(/-/g, ' ')}
                </button>
              ))}
            </div>
          )}
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden', background: '#fff', marginBottom: 12 }}>
              {newLines.map((l, i) => (
                <div key={`${l.familyId}-${l.colour}`} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderTop: i ? '1px solid #f3f4f6' : 'none', fontSize: 12 }}>
                  ...
                  <input type="number" min="1" value={l.quantity} onChange={e => setNewLines(newLines.map((x, j) => j === i ? { ...x, quantity: parseInt(e.target.value) || 1 } : x))} className="crm-input" style={{ width: 50, fontSize: 12, textAlign: 'right' }} />
      ...
      {loading ? <div style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>Loading…</div> : transfers.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {transfers.map(t => {
            ...
            return (
              <div key={t.id} style={{ border: '1px solid #e5e7eb', borderRadius: 10, background: '#fff', padding: '14px 16px' }}>
